...
public:
    ...
    Node<T>* search(const T& value) {
        ...
        while (current != nullptr) {
            if (*current->data == value) {  // If the data matches, return the node
                return current;
            }
    ...
    void remove(const T& value) {
        ...
        while (current != nullptr) {
            if (*current->data == value) {  // If the value matches
                if (previous == nullptr) {    // If the node to remove is the head
                    head = current->next;       // Update the head to the next node
                } else {                      // Otherwise, bypass the node to remove
